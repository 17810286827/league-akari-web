/**
 * 玩家标签计算（任务 8 移植自原版 LeagueAkari `match-card/utils/tags.tsx`）
 * 与 web 的差异（其余 compute* 函数逐字照搬原版）：
...
/** 连杀次数 >1 时拼接 "label×count"（原版 times 文案） */
function times(label: string, count: number, t: TagTFunction) {
  if (count === 1) {
    return `${label}`
  }
...
/** 队伍聚合统计：compute* 系列消费的字段（原版由 data-adapter 从 participants 计算得出） */
interface TagTeamStats {
...
/** 全队聚合统计：在单队聚合基础上多 maxCs（补兵最高） */
type TagAllTeamStats = TagTeamStats & { maxCs: number }

/** 标签计算上下文（web 版显式声明；原版由 useMatchCard 推导） */
export interface TagContext {
  participant: MatchCardParticipant
  team: TagTeamStats
  teams: { allTeamStats: TagAllTeamStats }
  basicInfo: MatchCardBasicInfo
}

/**
 * 多杀标签：LCU 连杀统计为包含式（一次五杀同时计入四杀/三杀/双杀计数），
 * 故逐级减去高等级击杀数后再展示（quadra -= penta；triple -= quadra + penta；double -= 三者之和）
 */
export function computeMultikillTags({ participant }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 伤害标签：全场最高 / 队内最高（对照原版 computeDamageTags） */
export function computeDamageTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 承伤标签：全场最高 / 队内最高（对照原版 computeTakenTags） */
export function computeTakenTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 治疗标签：全场最高 / 队内最高（对照原版 computeHealTags） */
export function computeHealTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 拆塔标签：全场最高 / 队内最高（对照原版 computeTowerTags） */
export function computeTowerTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 护盾标签：全场最高 / 队内最高（对照原版 computeShieldTags） */
export function computeShieldTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 单杀标签：单杀 ≥2 展示（对照原版 computeSoloTags） */
export function computeSoloTags({ participant }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 金币标签：全场最高 / 队内最高（对照原版 computeGoldTags） */
export function computeGoldTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 补兵标签：全场最高补兵（对照原版 computeCsTags） */
export function computeCsTags({ participant, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 补兵压制标签：对线最大补刀差 ≥40 展示（对照原版 computeCsAdvantageTags） */
export function computeCsAdvantageTags({ participant }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 击杀标签：全场最高 / 队内最高（对照原版 computeKillsTags） */
export function computeKillsTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 击杀参与率标签：全场最高 / 队内最高（对照原版 computeKpTags） */
export function computeKpTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 击飞击杀标签：≥6 次展示（对照原版 computedKnockUpTags） */
export function computedKnockUpTags({ participant }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 控制时长标签：全场最高 / 队内最高（对照原版 computeCcTags） */
export function computeCcTags({ participant, team, teams }: TagContext, t: TagTFunction): PlayerTag[] {
...
/** 伤转率标签：全场最高 / 队内最高（对照原版 computeDamageGoldEfficiencyTags） */
export function computeDamageGoldEfficiencyTags(
...
/**
 * 塔下击杀标签（对照原版 computeTowerKillTags）：
 * 仅召唤师峡谷(11)与嚎哭深渊(12)计算，按对局时长换算击杀密度阈值
 */
export function computeTowerKillTags({ participant, basicInfo }: TagContext, t: TagTFunction): PlayerTag[] {
  const tags: PlayerTag[] = []
